...
const StatusUpdater: React.FC<{
  ...
  const handleChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (!next) return;
    setLoading(true);
    try { await onUpdate(booking.id, next); }
...
export const AdminDashboard: React.FC = () => {
  ...
  const stats = {
    ...
    revenue:   bookings.filter((b) => b.status === 'completed').reduce((s, b) => s + b.fare, 0),
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-end justify-between">
          <div>
            ...
            <p className="text-text-sub">Manage bookings and users</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard label="Ongoing"        value={stats.ongoing}   icon={<RefreshCw className="w-5 h-5 text-blue-400" />}    color="bg-blue-400/20" />
          ...
          {(['bookings', 'users'] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 rounded-lg text-sm font-medium transition-all capitalize ${
                activeTab === tab ? 'bg-brand-red/20 text-brand-red' : 'text-text-sub hover:text-white'
              }`}>
              {tab}
            </button>
          ))}
        ...
        {activeTab === 'bookings' && (
          <div>
            ...
            {isLoading ? (
              ...
              <GlassCard>
                <p className="text-center text-text-sub py-12">No bookings found.</p>
              </GlassCard>
            ) : (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start gap-4 justify-between">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-text-sub mt-1">
                            <span className="text-brand-red">↑</span> {b.pickupLocation}
                          </p>
                          <p className="text-sm text-text-sub">
                            <span className="text-brand-gray">↓</span> {b.dropLocation}
                          </p>
                          <div className="flex gap-4 mt-2 text-xs text-text-sub flex-wrap">
                            <span>{b.serviceType} {b.serviceDetail ? `· ${b.serviceDetail}` : ''}</span>
        ...
        {activeTab === 'users' && (
          <div className="space-y-3">
            {isLoading ? (
              ...
              <GlassCard>
                <p className="text-center text-text-sub py-12">No users found.</p>
              </GlassCard>
            ) : (
              users.map((u: AppUser) => (
                <GlassCard key={u.uid} className="p-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-text-sub truncate">{u.email}</p>
                      <p className="text-xs text-text-sub">{u.phone}</p>
                    </div>
