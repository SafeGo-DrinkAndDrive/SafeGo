// ─── src/pages/Login.tsx ──────────────────────────────────────────────────────
// Changes (Phase 2):
//   After Google login, check user.phone — if empty, go to /phone-setup first.
//   Email/password users always have a phone (collected at register), so they
//   skip phone-setup and go straight to /vehicle-setup or /booking.
// ─────────────────────────────────────────────────────────────────────────────
...
// Inline SVG Google logo (avoids adding an asset dep)
const GoogleIcon = () => (
...
export const Login: React.FC = () => {
  ...
  const {
    user, isAuthenticated, vehicleRegistered,
    login, loginWithGoogle, authError, clearError,
  } = useAuth();
  ...
  // Where the user originally tried to go (if they were redirected to login)
  ...
  useEffect(() => {
    ...
    // Google-auth users with no phone must collect it first
    if (!user.phone?.trim()) {
      navigate('/phone-setup', { replace: true });
      return;
    }
    ...
    // Go where the user originally intended, or fall back to /booking
    if (intendedFrom && intendedFrom !== '/login') {
      navigate(intendedFrom, { replace: true });
    } else {
      navigate('/booking', { replace: true });
    }
  }, [isAuthenticated, user, vehicleRegistered, navigate, intendedFrom]);
      ...
      >
        ...
        <GlassCard glowColor="red">
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-xs text-text-sub">or</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>
